package dev.hafez.nestswitch.license;
...
public class CheckLicense {
   ...
   */
  private static final String PRODUCT_CODE = "PNESTSWITCH";
  ...
  public static void requestLicense(final String message) {
    ...
    ApplicationManager.getApplication().invokeLater(() -> showRegisterDialog("PNESTSWITCH", message), ModalityState.nonModal());
  }
  ...
  private static void showRegisterDialog(final String productCode, final String message) {
    ...
    if (registerAction != null) {
      // Pre-243 call form: this plugin compiles against 2023.3, where AnActionEvent.createEvent
      // and ActionUiKind do not exist yet. Deprecated on newer IDEs but still present at runtime.
      registerAction.actionPerformed(
        AnActionEvent.createFromDataContext("", new Presentation(), asDataContext(productCode, message))
      );
    }
  }
